                        ...
                        <tbody>
                            @forelse ($pacientesEliminados as $paciente)
                                <tr wire:key="deleted-{{ $paciente->id }}" class="border-b border-gray-700 hover:bg-[#204060]">
                                    <th class="tiBody px-4 py-1 text-[14px] font-medium text-white whitespace-normal min-w-[200px]">
                                    ...
                                    <td class="tiBody px-4 py-1 text-[14px] text-center relative">
                                        {{-- Botón Restaurar --}}
                                        <button wire:click="restore({{ $paciente->id }})"
                                                onclick="confirm('¿Restaurar a {{ $paciente->apellido_nombre }}?') || event.stopImmediatePropagation()"
                                        ...
                                        @role('super-admin')
                                        <button wire:click="forceDelete({{ $paciente->id }})"
                                                onclick="confirm('¿Eliminar permanentemente a {{ $paciente->apellido_nombre }}?') || event.stopImmediatePropagation()"
                            ...
                            @empty
                                <tr>
                                    <td colspan="9" class="px-4 py-4 text-center text-gray-400">No hay pacientes en la papelera.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
