    ...
    public function up()
{
    Schema::create('entrevistas', function (Blueprint $table) {
        $table->id();
            ...
            $table->boolean('enfermedad_preexistente')->nullable();
        ...
        $table->foreignId('tipo_entrevista_id')
              ->nullable()
              ->constrained('tipo_entrevistas') // Nombre exacto de la tabla
              ->onDelete('cascade');

        $table->foreignId('actitud_entrevista_id')
              ->nullable()
              ->constrained('actitud_entrevistas')
              ->onDelete('cascade');

        $table->foreignId('indicacionterapeutica_id')
              ->nullable()
              ->constrained('indicacionterapeuticas') // Exactamente como en la migración
              ->onDelete('cascade');

        $table->foreignId('abordaje_id')
              ->nullable()
              ->constrained('abordajes')
              ->onDelete('cascade');

        $table->foreignId('portacion_id')
              ->nullable()
              ->constrained('portacions') // ¡Nota el nombre!
              ->onDelete('cascade');

        $table->foreignId('salud_mental_id')
              ->nullable()
              ->constrained('salud_mentales') // Asegúrate de que existe
              ->onDelete('cascade');

        $table->foreignId('estado_entrevista_id')
              ->nullable()
              ->constrained('estado_entrevistas')
              ->onDelete('cascade');
