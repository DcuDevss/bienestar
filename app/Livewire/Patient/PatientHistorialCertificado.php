...
class PatientHistorialCertificado extends Component
{
    ...
    public function openEditPicker()  { $this->editPickerOpen = true;  }
    public function closeEditPicker() { $this->editPickerOpen = false; }
    ...
    public function delete(Paciente $paciente){ $paciente->delete(); }
    public function setSortBy($f){
        if($this->sortBy === $f){ $this->sortDir = ($this->sortDir=="ASC")?'DESC':"ASC"; return; }
        $this->sortBy = $f; $this->sortDir = 'DESC';
    }
